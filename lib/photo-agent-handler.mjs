...
async function githubUpload(path, base64) {
...
async function uploadHero(request, response) {
  ...
  const path = `stock-images/${part}-${stamp}-${nonce}.jpg`;

  await githubUpload(path, parsed.base64);
  const rawUrl = `https://raw.githubusercontent.com/${IMAGE_REPO}/${IMAGE_BRANCH}/${path}`;
  return json(response, 200, { url: rawUrl, path });
}
