import { updateOrderNotificationStatus } from "./order-notification.mjs";
...
export async function updateSandboxOrderStatus(request, response, authorised) {
  ...
  const issueNumber = Number(updated.metadata?.ao_order_issue) || null;
  const notificationRepo = updated.metadata?.ao_order_repo || (issueNumber ? "robbd86/automation-outlet-site" : null);
  if (issueNumber) await updateOrderNotificationStatus(issueNumber, orderStatus, notificationRepo);
  return json(response, 200, { sessionId, orderStatus, notificationIssue: issueNumber, notificationRepo, sandbox: true });
}
