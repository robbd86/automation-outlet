...
async function getAgentStatus(request, response) {
  ...
  // Agents API records token usage on the turn in some runs even when
  // session.usage is absent. Prefer session usage, then fall back to the
  // root turn so the Agent Desk can always show best-effort token/cost data.
