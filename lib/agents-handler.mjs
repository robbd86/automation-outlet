...
- Keep research efficient: use no more than 2 focused web searches for a normal listing. Stop as soon as you have enough evidence to identify the exact part and set a realistic price. Do not repeatedly search the same part or reopen sources unless necessary.
...
- Description must be concise, professional, plain text, suitable for Automation Outlet and eBay, and normally under 90 words.
- Keep pricingNotes under 90 words, checks to at most 4 items, and SEO keywords to at most 8 items.
...
async function startAgent(request, response) {
  ...
  const contract = kind === "listing"
    ? "Return ONLY valid JSON with exactly these top-level keys: partNumber, brand, identification, category, condition, quantity, listingTitle, ebayTitle, description, recommendedPriceGbp, priceLowGbp, priceHighGbp, deliveryMode, status, seoKeywords, checks, pricingNotes, pricingConfidence, marketEvidence. marketEvidence items must contain source, url, priceGbp, condition, evidenceType, warrantyService."
    : "Return ONLY valid JSON with exactly these top-level keys: summary, recommendation, confidence, estimatedRetailGbp, estimatedTradeGbp, maxCashBuyGbp, suggestedConsignmentAskGbp, items, risks, nextSteps. Each items entry must contain partNumber, identification, quantity, realisticSaleGbpEach, tradeValueGbpEach, maxBuyGbpEach, notes.";
  const task = `Automation Outlet task input:\n${JSON.stringify(input)}\n\n${contract} No markdown or commentary.`;
  ...
  const session = await openai("/agents/sessions", {
    ...
    body: JSON.stringify({
      ...
      agent: {
        model: kind === "listing" ? "gpt-5.6-luna" : "gpt-5.6-terra",
        instructions: cfg.instructions,
        tools: [{
          type: "web_search",
          mode: "live",
          context_size: kind === "listing" ? "low" : "medium",
          location: { country: "GB", timezone: "Europe/London" }
        }],
        multi_agent: { enabled: false },
        reasoning: { effort: kind === "listing" ? "none" : "medium" },
        text: {
          verbosity: "low",
          format: { type: "text" }
        }
...
async function getAgentStatus(request, response) {
  ...
  return json(response, 200, { ...base, status: "idle", usage: rootTurn?.usage || session.usage || null, result });
}
