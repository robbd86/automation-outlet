...
export async function updateOrderNotificationStatus(issueNumber,orderStatus){
  ...
  return gh(`/issues/${n}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:`${dispatched?"DISPATCHED AO ORDER":"NEW AO ORDER"} · ${tail}`,state:dispatched?"closed":"open"})});
}
