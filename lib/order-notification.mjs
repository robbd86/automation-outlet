...
const DEFAULT_REPO="robbd86/automation-outlet-orders";
const LABEL="ao-order";

function cfg(repoOverride){return{token:process.env.AO_GITHUB_TOKEN,repo:repoOverride||process.env.AO_ORDERS_REPO||DEFAULT_REPO,assignee:process.env.AO_ORDER_ASSIGNEE||"robbd86"};}

export function orderRepoName(){return cfg().repo;}

async function gh(path,options={},repoOverride){
  const {token,repo}=cfg(repoOverride);
...
export async function updateOrderNotificationStatus(issueNumber,orderStatus,repoOverride){
  const n=Number(issueNumber);if(!Number.isInteger(n)||n<=0)return null;
  const issue=await gh(`/issues/${n}`,{},repoOverride);
...
export async function updateOrderNotificationRefundStatus(issueNumber,refund={},repoOverride){
  const n=Number(issueNumber);if(!Number.isInteger(n)||n<=0)return null;
  const issue=await gh(`/issues/${n}`,{},repoOverride);
  const eventId=String(refund.eventId||"").trim();
  const eventMarker=eventId?`<!-- AO_REFUND_EVENT:${eventId} -->`:"";
  if(eventMarker&&String(issue.body||"").includes(eventMarker))return issue;

  const full=Boolean(refund.full);
  const restocked=Boolean(refund.restocked);
  const amount=money(refund.amountRefunded,refund.currency||"gbp");
  const tail=String(issue.title||"AO order")
    .replace(/^(?:NEW AO ORDER|DISPATCHED AO ORDER|REFUNDED AO ORDER|PARTIAL REFUND AO ORDER)\s*·\s*/i,"");
  const titlePrefix=full?"REFUNDED AO ORDER":"PARTIAL REFUND AO ORDER";
  const headline=full?"Refunded Automation Outlet order":"Partially refunded Automation Outlet order";
  const status=full
    ?(restocked?"Refunded · stock returned to available inventory":"Refunded · stock return/review required")
    :"Partially refunded · stock unchanged pending review";

  let body=String(issue.body||"");
  body=body.replace(/^# .*Automation Outlet order/im,`# ${headline}`);
  body=body.replace(/^- \*\*Status:\*\* .*$/im,`- **Status:** ${status}`);
  body+=`\n\n## Refund\n- **Refunded:** ${amount}\n- **Inventory:** ${restocked?"Restocked automatically":"Not restocked automatically"}\n${eventMarker}\n`;

  return gh(`/issues/${n}`,{
    method:"PATCH",
    headers:{"Content-Type":"application/json"},
    body:JSON.stringify({
      title:`${titlePrefix} · ${tail}`,
      body,
      state:full?"closed":"open"
    })
  },repoOverride);
}
