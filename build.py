...
if _agent_file.exists():
    ...
    _event_start = _agent_source.find('  await openai(`/agents/sessions/${encodeURIComponent(session.id)}/events`, {')
    _event_end_marker = '\n\n  return json(response, 202, {'
    if _event_start >= 0:
        _event_end = _agent_source.find(_event_end_marker, _event_start)
        if _event_end >= 0:
            _agent_source = _agent_source[:_event_start] + _agent_source[_event_end + 2:]
    ...
    _agent_file.write_text(_agent_source, encoding="utf-8")
...
// Keep the hard blocklist deliberately narrow: only identities confirmed as fraudulent.
// Everything else is still delivered, with suspicious procurement/credit wording merely flagged for review.
const blockedDomains = new Set(["danthermgroups.com"]);
const blockedPhones = new Set(["07828705752"]);
...
function reviewReasons(data) {
  ...
  const reasons = [];
  if (/\bnet\s*(30|45|60|90)\b/.test(text)) reasons.push("credit terms mentioned");
  if (/\b(payment terms?|credit account|upon (satisfactory )?delivery|procurement director|vendor registration|official quotation)\b/.test(text)) {
    reasons.push("procurement / post-delivery payment wording");
  }
...
form.addEventListener("submit", async function(e){
  ...
  const domain = emailDomain(data.get("email"));
  const phone = normalisePhone(data.get("phone"));

  // Silently accept known scam submissions so the sender is not encouraged to keep changing details.
  if (blockedDomains.has(domain) || blockedPhones.has(phone)) {
    form.reset();
    statusEl.style.color = "var(--blue-bright)";
    statusEl.textContent = "Thanks — your details are on their way. We'll be in touch, usually the same working day.";
    return;
  }

  const reasons = reviewReasons(data);
  if (reasons.length) {
    data.append("ao_review", "CHECK BEFORE OFFERING CREDIT OR SHIPPING: " + reasons.join("; "));
    const subject = String(data.get("_subject") || "Automation Outlet — general enquiry");
    data.set("_subject", "[REVIEW] " + subject);
  } else {
    data.append("ao_review", "No automated fraud flags");
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      statusEl.textContent = "Something went wrong — please email us directly at info@automation-outlet.co.uk.";
    }
  } catch (err) {
    statusEl.style.color = "#ff8080";
    statusEl.textContent = "Network error — please email us directly at info@automation-outlet.co.uk.";
  } finally {
